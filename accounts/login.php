...
// Redirect if already logged in and session active
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        try {
            // Only fetch users who are approved
            $stmt = $conn->prepare("SELECT user_id, user_name, email, password, is_admin, is_approved 
                                    FROM users_table 
            ...
            if ($user) {
                ...
                $error = "Please wait for an admin approval, Check again in your SMS.";
            }
