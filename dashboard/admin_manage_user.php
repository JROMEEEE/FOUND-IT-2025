...
function userHasDependencies($conn, $user_id)
{
    $tables = [
        "lost_report" => "user_id",
        "found_report" => "user_id",
        "claim_request" => "user_id",
        "user_admin_msgs" => ["sender_id", "receiver_id"]
    ];
    ...
    foreach ($tables as $table => $columns) {
        if (is_array($columns)) {
            foreach ($columns as $col) {
                $stmt = $conn->prepare("SELECT COUNT(*) FROM $table WHERE $col = ?");
                $stmt->execute([$user_id]);
                $count = $stmt->fetchColumn();
                if ($count > 0) {
                    $dependencies[] = "$count record(s) in $table";
                }
            }
        } else {
            $stmt = $conn->prepare("SELECT COUNT(*) FROM $table WHERE $columns = ?");
            $stmt->execute([$user_id]);
            $count = $stmt->fetchColumn();
            if ($count > 0) {
                $dependencies[] = "$count record(s) in $table";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['user_id'])) {
    ...
    if ($action === 'approve') {
        ...
        $dependencies = userHasDependencies($conn, $user_id);

        if (!empty($dependencies)) {
            $msg = "⚠ Cannot delete user. They have: " . implode(", ", $dependencies) . ".";
