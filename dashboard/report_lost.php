...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!is_dir($uploadDir)) {
     ...
     $image_path = null; //variable to store image path 
    if (!empty($_FILES['lost_image']['name'])) {
        $fileName = basename($_FILES['lost_image']['name']);
        ...
        if (in_array($imageFileType, $allowedTypes)) {
            if (move_uploaded_file($_FILES['lost_image']['tmp_name'], $targetFile)) {
                ...
                $image_path = 'uploads/lost_items/' . time() . '_' . $fileName;
            } else {
    ...
    $sql = "INSERT INTO lost_report 
            (lost_name, lost_desc, location_id, lost_datetime, user_id, image_path, category_id, lost_status)
    ...
    $stmt->bindParam(':lost_status', $lost_status);
     ...
     if (empty($error)) {
        if ($stmt->execute()) {
            $success = "Lost item successfully reported!";
        } else {
