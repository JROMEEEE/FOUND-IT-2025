...
$query = "
    SELECT d.fnd_id, d.fnd_name, d.fnd_desc, d.fnd_datetime, d.fnd_status, d.image_path, c.category_name, l.location_name
    ...
    <table id="decayedTable" class="table table-striped table-hover align-middle">
      ...
      <tbody>
        <?php foreach ($decayed_items as $item): ?>
          <?php
              $thumb = (!empty($item['image_path']) && file_exists('../' . $item['image_path']))
                       ? '../' . $item['image_path']
          ...
          <tr>
            ...
            <td class="text-center">
              <img src="<?= htmlspecialchars($thumb) ?>" class="img-thumbnail discard-img" style="max-width:80px; max-height:80px; cursor:pointer;">
            </td>
            <td>
    ...
    <div class="modal-content border-0 shadow">
      ...
      <div class="modal-body" id="discardDetailsContent">
        ...
        <p class="small text-muted mb-2">
          <strong>Disclaimer:</strong> Certain discarded items are redirected to the appropriate offices as follows:
        </p>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content bg-transparent border-0">
      <div class="modal-body p-0 text-center">
        <img id="zoomedImage" src="" class="img-fluid rounded shadow-sm">
      </div>
...
$(document).ready(function () {
    ...
    // Zoom image on click
    $(document).on('click', '.discard-img', function() {
